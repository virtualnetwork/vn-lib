 ...
 *
 * SVN $Revision: 5341 $
 * SVN $Date: 2013-03-26 14:36:39 +0530 (Tue, 26 Mar 2013) $
 */
/*=========================================================================*//**
  ...
  @section intro_sec Introduction
   ...
   The CAN controller shall be configurable to provide support up to 32 transmit 
  ...
  using the Actel Firmware Catalog.
   ...
   Interrupt Support
     ...
     functions to the respective interrupt registers are provided. The individual 
     interrupt enable bits can be set using MSS_CAN_Set_int_ebl() function  while 
     MSS_CAN_get_int_ebl() function  returns their actual state. 
     MSS_CAN_set_global_int_ebl() function enables global interrupt generation by   
     the core while MSS_CAN_get_global_int_ebl() function indicates if interrupt   
   ...
   Helper Functions
     The CAN core expects all ID bits to be left aligned. This makes setting the 
     ID cumbersome. Using MSS_CAN_set_id() function , a given right-aligned 
     ID field is modified according to the ID size which is indicated by the IDE  
     bit. MSS_CAN_get_id() function  provides the reverse operation: It returns  
     the ID right aligned. MSS_CAN_get_msg_filter_mask() function  packs the ID,  
     IDE, and RTR bits together as they are used in the mask registers.
     MSS_CAN_get_mask_n() function returns the message filter settings of the 
     selected receive mailbox. MSS_CAN_set_mask_n() function configures the 
     message filter settings for the selected receive mailbox.
...
typedef union can_config_reg
{
    ...
    struct
    {
        ...
        __IO uint32_t ENDIAN:1;        /* 12 */
        __IO uint32_t N_A0:3;          /* 15..13 */
        __IO uint32_t CFG_BITRATE:8;   /* 23..16 */
        __IO uint32_t N_A1:8;          /* 31..24 */
    };
} CAN_CONFIG_REG;
...
/*-------------------------------------------------------------------------*//**
  ...
  Each CAN core instance has its own mss_can_instance_t structure that contains
...
/*-------------------------------------------------------------------------*//**
  ...
  @param bitRate
    The bitRate parameter is used to configure CAN speed. Following default 
    bitrate definitions might be used.
  ...
  @param pcan_config
    The pcan_config parameter is a pointer to a configuration structure. This 
    structure is only used when bitRate configured as CAN_SPEED_MANUAL.
  ...
  int main(void)
  {
    ...
    CANReg.CFG_BITRATE = MSS_CAN_CalcClkDivisor(SYSTEM_CLOCK, 8, 500000);
    CANReg.CFG_TSEG1 = 4;
    CANReg.CFG_TSEG2 = 1;
...
/*-------------------------------------------------------------------------*//**
   ...
   and alsochecks for RTR auto-reply is enabled or not.
...
/*-------------------------------------------------------------------------*//**
  ...
  Thefunction verifies that this mailbox is configured for Full CAN operation  
  and is empty.
