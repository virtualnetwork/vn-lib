package VN.Message.Probe_Request is
   ...
   for VN_Message_Probe_Request use record
      ...
      Unknown_Payload   at 0 range (HEADER_SIZE + CUUID_SIZE) * 8 ..
        (HEADER_SIZE + CUUID_SIZE + PROBE_REQUEST_UNKNOWN_PAYLOAD_SIZE) * 8 - 1;

      Checksum          at 0 range (HEADER_SIZE + CUUID_SIZE +
                                      PROBE_REQUEST_UNKNOWN_PAYLOAD_SIZE) * 8 ..
          (HEADER_SIZE + CUUID_SIZE + PROBE_REQUEST_UNKNOWN_PAYLOAD_SIZE + CHECKSUM_SIZE) * 8 - 1;
   end record;
